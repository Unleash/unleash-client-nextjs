...
exports.removeTrailingSlash = exports.getDefaultClientConfig = exports.randomSessionId = exports.safeCompare = void 0;
/**
 ...
 */
const safeCompare = function safeCompare(a, b) {
    let strA = String(a);
    let strB = String(b);
...
const validateEnviromentVariables = () => {
    if (process.env.NEXT_PUBLIC_UNLEASH_SERVER_API_TOKEN) {
        console.warn("You are trying to set `NEXT_PUBLIC_UNLEASH_SERVER_API_TOKEN`. Server keys shouldn't be public. Use frontend keys or skip `NEXT_PUBLIC_ prefix.");
    }
};
const getFrontendUrl = () => process.env.UNLEASH_FRONTEND_API_URL ||
    process.env.NEXT_PUBLIC_UNLEASH_FRONTEND_API_URL;
const getServerBaseUrl = () => process.env.UNLEASH_SERVER_API_URL ||
...
const removeTrailingSlash = (url) => url && url.replace(/\/$/, "");
